FluidBufAmpCurve : FluidBufProcessor {
	...
	*kr  { |source, startFrame = 0, numFrames = -1, startChan = 0, numChans = -1, features, fastRampUp = 1, fastRampDown = 1, slowRampUp = 100, slowRampDown = 100, floor = -144, highPassFreq = 85, trig = 1, blocking = 0| 
        ...
        source.isNil.if {"FluidBufAmpCurve:  Invalid source buffer".throw};
        features.isNil.if {"FluidBufAmpCurve:  Invalid features buffer".throw};
                
		^FluidProxyUgen.kr(\FluidBufAmpCurveTrigger, -1, source, startFrame, numFrames, startChan, numChans, features, fastRampUp, fastRampDown, slowRampUp, slowRampDown, floor, highPassFreq, trig, blocking);
	}
...
FluidBufAmpCurveTrigger : FluidProxyUgen {}
